#!/usr/bin/env python3
"""
Final Ranking Script
Replicates the "ALL MODELS - RANKED BY ROC-AUC" table from the original notebook.
Evaluates all 5 Optimized Base Models + Voting + Stacking.
"""
import pandas as pd
import numpy as np
...
def get_metrics_dict(model, X_test, y_test):
...
def main():
    ...
    for config in configs:
        ...
        print(f"\nEvaluating All Models for Region: {region}...")
        ...
        # Define Base Learners (Optimized)
        ...
        stacking_clf = StackingClassifier(estimators=estimators, final_estimator=LogisticRegression(random_state=42), cv=5, n_jobs=-1)
        ...
        for name, model in models_to_run:
            ...
            metrics = get_metrics_dict(model, X_test, y_test)
            metrics['Model'] = name
            results_list.append(metrics)
        
        # Create DataFrame and Ranking
        df_summary = pd.DataFrame(results_list)
        df_summary = df_summary[['Model', 'Accuracy', 'Precision', 'Recall', 'F1-Score', 'ROC-AUC']]
        df_summary = df_summary.sort_values('ROC-AUC', ascending=False)
        
        print("\n" + "="*80)
        print(f"ALL {region.upper()} MODELS - RANKED BY ROC-AUC")
        print("="*80)
