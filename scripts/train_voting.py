#!/usr/bin/env python3
"""
Voting Ensemble Training Script
Builds a VotingClassifier (Soft Voting) using optimized RF, LR, and GB base learners.
"""
import pandas as pd
import numpy as np
...
def main():
    ...
    for config in configs:
        ...
        # Evaluate Voting
        metrics_voting = get_metrics(voting_clf, X_test, y_test, f"Voting Ensemble ({region})")
        ...
        # Print Diff
        print("\nImprovement with Voting:")
        print(f"ROC-AUC: {metrics_voting['ROC-AUC'] - metrics_gb['ROC-AUC']:+.4f}")
        print(f"F1-Score: {metrics_voting['F1-Score'] - metrics_gb['F1-Score']:+.4f}")
